...
//TODO~
#define OV8858_TEST_PATTERN_CHECKSUM 0x899a55db
...
/*****************************  OTP Feature  **********************************/
#define OV8858_USE_OTP
#if defined(OV8858_USE_OTP)
...
static int RG_Ratio_Typical = 0x138;
static int BG_Ratio_Typical = 0x11A;
...
static int check_otp_VCM(int index)
{
    ...
    mdelay(5);
    //select group
    flag = OV8858_read_cmos_sensor(0x7030);
    if(index==1)
    {
        flag = (flag>>6) & 0x03;
    }
...
static int read_otp_info(int index, struct otp_struct *otp_ptr)
{
    ...
    mdelay(5);
    (*otp_ptr).module_integrator_id = OV8858_read_cmos_sensor(address_start);
    (*otp_ptr).lens_id = OV8858_read_cmos_sensor(address_start + 1);
    (*otp_ptr).production_year = OV8858_read_cmos_sensor(address_start + 2);
    (*otp_ptr).production_month = OV8858_read_cmos_sensor(address_start + 3);
    (*otp_ptr).production_day = OV8858_read_cmos_sensor(address_start + 4);
    // clear otp buffer
    for(i=address_start;i<=address_end;i++) {
        OV8858_write_cmos_sensor(i, 0x00);
    }
...
static int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
    ...
    OV8858_write_cmos_sensor(0x3d81, 0x01); // load otp into buffer
    mdelay(5);
    temp = OV8858_read_cmos_sensor(address_start + 4);
    (*otp_ptr).rg_ratio = (OV8858_read_cmos_sensor(address_start )<<2) + ((temp>>6) & 0x03);
    (*otp_ptr).bg_ratio = (OV8858_read_cmos_sensor(address_start + 1)<<2) + ((temp>>4) & 0x03);
    (*otp_ptr).light_rg = (OV8858_read_cmos_sensor(address_start + 2) <<2) + ((temp>>2) & 0x03);
    (*otp_ptr).light_bg = (OV8858_read_cmos_sensor(address_start + 3)<<2) + (temp & 0x03);
    // clear otp buffer
    for(i=address_start;i<=address_end;i++) {
        OV8858_write_cmos_sensor(i, 0x00);
    }
...
static int read_otp_VCM(int index, struct otp_struct * otp_ptr)
{
    ...
    temp = OV8858_read_cmos_sensor(address_start + 2);
    (* otp_ptr).VCM_start = (OV8858_read_cmos_sensor(address_start)<<2) | ((temp>>6) & 0x03);
    (* otp_ptr).VCM_end = (OV8858_read_cmos_sensor(address_start + 1) << 2) | ((temp>>4) & 0x03);
...
static void OV8858_Sensor_Init(void)
{
	...
	//	  ;;
		...
		OV8858_write_cmos_sensor(0x0100, 0x01); 	//	OV8858_write_cmos_sens0x;0x);	  //

#ifdef OV8858_USE_OTP
